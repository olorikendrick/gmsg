...
pub fn run(repository:Repository)->anyhow::Result<()>{
    ...
    let diff = repository.diff_tree_to_index(tree.as_ref(), index.as_ref(), None).context("Could not get diff")?;
    diff.print(DiffFormat::Patch, |_,_,line|{
        out.push_str(
            str::from_utf8(line.content()).context("Could not parse diff").unwrap()
        );
        true
    }).unwrap();
     println!("{}",out);
